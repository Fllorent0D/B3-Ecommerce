...
import static server_dismap.protocoleDISMAP.REQUEST_INTERRUPT;
...
public class RunnableINDEP implements Runnable, protocoleINDEP {
    ...
    private BeanDBAccessOracle beanDecision = null;
    ...
    public RunnableINDEP(Socket s) {
        ...
        beanDecision = new BeanDBAccessOracle("127.0.0.1", "22223", "DECISIONS", "oracle");

        try {
            beanOracle.connect();
            beanDecision.connect();
    ...
    private void executeDescrCont(Message request) {
        ...
        Message response = new Message();
        REXP rResponse;
        query =    "select to_char(fa.date_facturation, 'DD/MM/YYYY') as jour, sum(tot.total) as total, count(*) as nbr\n" +
                    "from (select * \n" +
                    "      from factures\n" +
        ...
        try {
            ...
            while (result.next()) {
                if(result.getInt(3)>higher)
                {
                    mode = result.getString(1);
                    higher = result.getInt(3); 
                }
            ...
            /* mode ? */
            response.addParam("mode", mode);
            response.addParam("status", 1);
        
            beanDecision.executeQuery("INSERT INTO RESULTATS_DESCRCONT (nbr, moyenne, ecarttype, mediane, modee) values ("+nbrElem+","+response.getParam("moyenne")+","+response.getParam("ecart-type")+","+response.getParam("mediane")+", '"+mode+"')");
    ...
    private void executeVenteRep(Message request) {
        ...
        try {
            ...
            String bdresult = "{";
            while (result.next()) {
                bdresult += "{"+result.getString("type")+":"+result.getDouble("pct")+"},";
                response.addParam(result.getString("type"), result.getDouble("pct"));
            }
            bdresult = bdresult.substring(0, bdresult.length()-1) + "}";

            response.addParam("status", 1);
            beanDecision.executeQuery("INSERT INTO RESULTATS_VENTES_REP (annee, mois, resultat) values ("+annee+","+mois+",'"+bdresult+"')");
    ...
    private void executeVentesComp(Message request) {
        ...
        try {
            ...
            while (result.next()) {
                ...
                bdresult += "{"+tmp.toString()+":"+result.getDouble("total")+"},";
            ...
            response.addParam("status", 1);
            beanDecision.executeQuery("INSERT INTO RESULTATS_VENTES_COMP (annee, type, resultat) values ("+annee+","+nbr+",'"+bdresult+"')");
    ...
    private void executeTestConf(Message request) {
        ...
        try {
            ...
            {    
                ...
                beanDecision.executeQuery("INSERT INTO RESULTATS_INFER_TEST_CONF (nbr, pvalue) values ("+nbrElem+","+pv+")");
    ...
    private void executeTestAnova(Message request) {
        ...
        try {
            ...
            pv = connexionR.eval("summary(aov (montant ~  ville, data = data.frame(montant,ville)))[[1]][[\"Pr(>F)\"]][[1]]").asDouble();
            
            /* Code badass remplacé par le [[1]][[\"Pr(>F)\"]][[1]] plus haut */
            /*
            REXP element;
            testList = rResponse.asList();
            ...
            */
            ...
            response.addParam("status", 1);
             beanDecision.executeQuery("INSERT INTO RESULTATS_INFER_TEST_ANOVA (nbr, pvalue) values ("+nbrElem+","+pv+")");
